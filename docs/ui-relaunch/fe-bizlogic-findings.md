...
- 证据: 前端 frontend/app/src/screens/ProductDetail.jsx:175(isCard 只认 1)、:176(out=false)、:184-188(stockBadge=现货)、:192-196(maxBuy 不含 stock)、:563(disabled 仅 out);后端 app/model/Product.php:94-97(usesCardPool 含 RIGHTS=4)、app/service/OrderService.php:106-122(权益类同样取卡、不足报 STOCK_NOT_ENOUGH)、app/service/StorefrontService.php:151(返回真实 stock)
...
- 证据: 前端 frontend/app/src/screens/OrderLookup.jsx:311(OrderResult 形参),:483-500(下载块),:489 `{!directMode && (`,:490 `loading={loading} onClick={search}`,:497 `{directMode ? ...}`;父作用域定义 :64,:71,:81。后端 app/service/DownloadService.php:42-43,:65;app/service/BuyerOrderService.php:46-49
...
- 证据: 前端列表(错):frontend/app/src/screens/StorefrontHome.jsx:676-696(stock=Number(p.stock);<ProductCard stock={stock} …/>,未判 show_stock_type);ProductCard:frontend/design-system/components/commerce/ProductCard.jsx:55-62(out=stock<=0→「已售罄」;low=stock<=5→「仅剩 {stock}」)。前端详情(对的口径):frontend/app/src/screens/ProductDetail.jsx:181-187 与 434-438(showExactStock=Number(p.show_stock_type)===1 才显示「仅剩 N 件」)。后端真值:app/service/StorefrontService.php:52(列表下发 show_stock_type)、160(详情下发)。
...
- 证据: 前端 frontend/app/src/screens/ProductDetail.jsx:196(`p.max_buy>0 ? p.max_buy : 99`)、:435-441(库存提示仅对 isCard);后端 app/service/OrderService.php:101-122
...
- 证据: 前端 frontend/app/src/screens/ProductDetail.jsx:176、:232-249(submit 失败仅 setSubmitErr,不重拉)、:563;后端 app/service/OrderService.php:119-121、docs/spec.md:158,391
...
- 证据: 前端 frontend/app/src/screens/OrderLookup.jsx:318 `const canComplain = [STATUS.DELIVERED, STATUS.EXCEPTION, 4].includes(statusNum);`;后端 app/service/ComplaintService.php:19,:26-27
...
- 证据: 前端 frontend/app/src/console/merchant/Coupons.jsx:116-119(else 分支:const min=Number(form.min_amount)||0; if(min>0 && v>min){setFormErr('满减券的减免金额不能超过使用门槛');return;});后端 app/service/CouponService.php:96-100(满减分支仅 if Money::cmp(value,'0')<=0 抛错)+ 90-101 整个 validateValueByType 无 value vs min_amount 校验;封顶逻辑 CouponService.php:182-189
...
- 证据: 前端 frontend/app/src/console/admin/Settings.jsx:328-336(withdraw_fee_rate 配置项,rate 提示生效)+ Wallet.jsx:206-212(手续费列)+ Withdrawals.jsx:131-134(手续费列);后端 app/service/MerchantWalletService.php:78('fee' => '0.00')与 AdminWithdrawService.php:75-90、105-129(approve/reject 仅用 amount,无 fee/withdraw_fee_rate 计算)
...
- 证据: 前端:frontend/app/src/screens/StorefrontHome.jsx:391-392(allCount=list.length;tabs 含 {全部,goods_count:allCount});405-407(byCat 再 byType 叠加 typeFilter 过滤);590-606(分类 chip 渲染 c.goods_count,不随 typeFilter 重算)。后端:app/service/StorefrontService.php:57-63(countByCat 不区分 goods_type)、73-80(goods_count 下发)。
...
- 证据: 前端:frontend/app/src/console/merchant/Stats.jsx:87-99(同卡:value=sales_today,sub 含『累计 sales』『昨日 sales_yesterday』并对 today/yesterday 算 Delta)。后端:app/service/MerchantStatsService.php:21(sales 用 rangeQuery=PAID+DELIVERED)、31-33(sales_today/yesterday/month 用 deliveredQuery=DELIVERED-only)、17(PAID_STATUSES 定义)。
...
- 证据: 前端 frontend/app/src/console/merchant/Coupons.jsx:114-115(if Number(form.type)===TYPE_PERCENT){ if(!(v>=1 && v<=99)){setFormErr('折扣值需在 1~99 之间')...}});Promotions.jsx:166(Input min="1" max="99");后端 app/service/CouponService.php:92-94(Money::cmp(value,'0')<=0 || Money::cmp(value,'100')>=0 才抛错)与 PromotionService.php:75-78 同
