...
# 第六轮:新特性独立安全审查(买家账号/发货邮件/卡密加密)

新特性交付后做独立对抗式审查(3 finder × disprove),9候选→6确认,二次研判:**2 项真实已修,4 项研判为可接受/已缓解**。
(另:我自审已先修了 critical 的「我的订单按邮箱认领」IDOR,见 commit 854c0a0。)

## 确认并已修(2)
| # | 严重度 | 缺陷 | 处置 |
|---|---|---|---|
| 1 | high | 买家注册 check-then-create 竞态:并发同邮箱命中唯一约束 → 裸 500 | register 捕获 1062 → 干净 STATE_INVALID |
| 2 | high | `POST /admin/settings` **回显** smtp_pass 明文(GET 已脱敏,POST 漏了) | 敏感键设新值后返回 `(set)` 不回显;测试断言 |

## 二次研判为可接受 / 已缓解(4)
- **#3 SSRF smtp_host**:需 admin(可信运维)且**本地 relay(127.0.0.1:25)是常见合法部署**,封私网 IP 反而破坏正常用法。不封。
- **#4 卡密邮件明文 / secure=none**:邮件发卡密是**对标卡网的有意特性**(独角数卡同样如此);`none` 对本地 relay 合法,且 UI 已提示「不推荐」,属 admin 知情选择。
- **#5 SMTP 异常日志含 host/响应**:运维自有日志,细节助排错;非对外泄漏。
- **#6 GCM 解密失败返回密文**:已在 commit c7164fd 加告警日志使其非静默;发货路径抛异常会致网关无限重投(更糟),保留返回+告警是最不坏解。
