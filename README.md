...
仅实现 **易支付/MD5** 渠道(支付宝/微信原生驱动预留接口);游客下单;卡密去重(可选 `CARD_SECRET_KEY` 开启 AES-256-GCM 落库加密)。买家账号体系为预留项。**支付发起→回调发卡全链路、退款闭环、负欠隔离、限量券下单即占额、异常单处置、发货邮件通知(可配 SMTP)、卡密落库加密等已实现**;经网关退款给买家、多支付渠道等需接入对应外部能力(见 `docs/blockers.md`)。
