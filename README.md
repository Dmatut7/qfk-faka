...
- **任务与验收**:`docs/tasks.md`
- **范围决策**:`docs/blockers.md`(原始默认与后续实现进度;退款/发货通知/买家账号/卡密加密均已实现,仅支付宝/微信原生驱动仍预留)
- **开发铁规则**:`CLAUDE.md`

## 接入真实易支付收款

代码已实现彩虹易支付(MD5)**全链路**:发起支付按 `submit.php` 页面跳转模式构建签名参数(`EpayDriver::buildPay`),前端携带完整参数跳转聚合收银台,网关异步回调走 `NotifyService`(验签 + 金额校验 + 幂等)自动发卡。**接入真实网关只需配置渠道,无需改代码**:

1. 平台后台登录 → **渠道管理** → 新建/编辑 `epay` 渠道,填入你的易支付:
   - `pid`:商户 ID;`key`:商户密钥;`gateway`:易支付站点根地址(如 `https://pay.yoursite.com`,程序自动拼 `/submit.php`)。
2. 启用该渠道。买家下单 → 去支付 → 跳转你的易支付收银台(微信/支付宝)→ 付款 → 自动发货。
3. 回调地址由系统自动下发为 `{你的域名}/pay/notify/epay`,确保易支付侧能访问到该地址。

> 演示数据里的 `epay` 渠道指向占位网关 `pay.example.com`,**仅供本地走通流程**,上线务必改成你自己的易支付参数。
> 支付宝/微信**原生**驱动为预留接口(易支付聚合已覆盖二者收款,通常无需单独接)。

## 本期范围

仅实现 **易支付/MD5** 渠道(支付宝/微信原生驱动预留接口,易支付已聚合二者收款);游客下单,亦支持**可选买家账号**(注册/登录/我的订单);卡密去重(可选 `CARD_SECRET_KEY` 开启 AES-256-GCM 落库加密)。**支付发起→回调发卡全链路、退款闭环、负欠隔离、限量券下单即占额、异常单处置、发货邮件通知(可配 SMTP)、卡密落库加密、买家账号等均已实现**;经网关退款给买家、支付宝/微信原生驱动等需接入对应外部能力(见 `docs/blockers.md`)。
