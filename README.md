...
仅实现 **易支付/MD5** 渠道(支付宝/微信原生驱动预留接口);游客下单;卡密明文+去重。发货通知(邮件/站内)、买家账号、卡密加密落库为预留项。**支付发起→回调发卡全链路、退款闭环、负欠隔离、限量券下单即占额、异常单处置等已实现**;经网关退款给买家、多支付渠道等需接入对应外部能力(见 `docs/blockers.md`)。
